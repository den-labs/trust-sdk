...
import { Ayni, AuthenticationError } from '../src'

const apiKey = process.env.AYNI_API_KEY
const chainId = Number(process.env.AYNI_CHAIN_ID ?? 43113) // Fuji
const agentId = Number(process.env.AYNI_AGENT_ID ?? 74)

describe.skipIf(!apiKey)('Ayni E2E (live endpoints)', () => {
  let client: Ayni

  beforeAll(() => {
    client = new Ayni({ apiKey: apiKey!, timeoutMs: 15000 })
  })
  ...
  it('invalid API key returns AuthenticationError', async () => {
    const badClient = new Ayni({ apiKey: 'ds_invalid_key', timeoutMs: 10000 })
